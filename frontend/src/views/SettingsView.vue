      ...
      <div class="form-group avatar-section">
        ...
        <div class="avatar-row">
          ...
          <div class="avatar-actions">
            ...
            <!-- Preset frame dropdown -->
            <template v-if="frameBorderType === 'preset'">
              <div class="dropdown-wrap" ref="presetDropdownRef">
                <button type="button" class="dropdown-trigger" :aria-expanded="presetDropdownOpen" @click="presetDropdownOpen = !presetDropdownOpen">
                  <span>{{ PRESET_LABELS[presetName] }}</span>
                  <i class="pi pi-chevron-down dropdown-chevron" aria-hidden="true"></i>
                </button>
                <Transition name="dropdown">
                  <div v-if="presetDropdownOpen" class="dropdown-panel" role="menu">
                    <button
                      ...
                      @click="presetName = key as typeof presetName; presetDropdownOpen = false"
                    >{{ label }}</button>
                  </div>
                </Transition>
              </div>
            ...
            <!-- Badge dropdown -->
            <div class="dropdown-wrap" ref="badgeDropdownRef">
              <button type="button" class="dropdown-trigger" :aria-expanded="badgeDropdownOpen" @click="badgeDropdownOpen = !badgeDropdownOpen">
                <span>{{ BADGE_LABELS[badge] }}</span>
                <i class="pi pi-chevron-down dropdown-chevron" aria-hidden="true"></i>
              </button>
              <Transition name="dropdown">
                <div v-if="badgeDropdownOpen" class="dropdown-panel" role="menu">
                  <button
                    v-for="(label, key) in BADGE_LABELS"
                    :key="key"
                    type="button"
                    class="dropdown-option"
                    role="menuitem"
                    :class="{ active: badge === key }"
                    @click="badge = key as typeof badge; badgeDropdownOpen = false"
                  >{{ label }}</button>
                </div>
              </Transition>
            </div>
            ...
            <template v-if="badge !== 'none'">
              <label class="avatar-shape-label">Badge position</label>
              <div class="dropdown-wrap" ref="badgePosDropdownRef">
                <button type="button" class="dropdown-trigger" :aria-expanded="badgePosDropdownOpen" @click="badgePosDropdownOpen = !badgePosDropdownOpen">
                  <span>{{ BADGE_POSITION_LABELS[badgePosition] }}</span>
                  <i class="pi pi-chevron-down dropdown-chevron" aria-hidden="true"></i>
                </button>
                <Transition name="dropdown">
                  <div v-if="badgePosDropdownOpen" class="dropdown-panel" role="menu">
                    <button
                      v-for="(label, key) in BADGE_POSITION_LABELS"
                      :key="key"
                      type="button"
                      class="dropdown-option"
                      role="menuitem"
                      :class="{ active: badgePosition === key }"
                      @click="badgePosition = key as typeof badgePosition; badgePosDropdownOpen = false"
                    >{{ label }}</button>
                  </div>
                </Transition>
              </div>
            ...
            <!-- Animation dropdown -->
            <div class="dropdown-wrap" ref="animationDropdownRef">
              <button type="button" class="dropdown-trigger" :aria-expanded="animationDropdownOpen" @click="animationDropdownOpen = !animationDropdownOpen">
                <span>{{ ANIMATION_LABELS[frameAnimation] }}</span>
                <i class="pi pi-chevron-down dropdown-chevron" aria-hidden="true"></i>
              </button>
              <Transition name="dropdown">
                <div v-if="animationDropdownOpen" class="dropdown-panel" role="menu">
                  <button
                    v-for="(label, key) in ANIMATION_LABELS"
                    :key="key"
                    type="button"
                    class="dropdown-option"
                    role="menuitem"
                    :class="{ active: frameAnimation === key }"
                    @click="frameAnimation = key as typeof frameAnimation; animationDropdownOpen = false"
                  >{{ label }}</button>
                </div>
              </Transition>
            </div>
...
const badgeDropdownOpen = ref(false)
const badgePosDropdownOpen = ref(false)
const animationDropdownOpen = ref(false)

// Dropdown refs for click-outside
const presetDropdownRef = ref<HTMLElement | null>(null)
const badgeDropdownRef = ref<HTMLElement | null>(null)
const badgePosDropdownRef = ref<HTMLElement | null>(null)
const animationDropdownRef = ref<HTMLElement | null>(null)

function onDocumentClick(e: MouseEvent) {
  const target = e.target as Node
  if (presetDropdownRef.value && !presetDropdownRef.value.contains(target)) presetDropdownOpen.value = false
  if (badgeDropdownRef.value && !badgeDropdownRef.value.contains(target)) badgeDropdownOpen.value = false
  if (badgePosDropdownRef.value && !badgePosDropdownRef.value.contains(target)) badgePosDropdownOpen.value = false
  if (animationDropdownRef.value && !animationDropdownRef.value.contains(target)) animationDropdownOpen.value = false
}
...
/* ── Dropdown (matches WriteView) ───────────────────────── */
.dropdown-wrap { position: relative; margin-top: 0.35rem; }
.dropdown-trigger {
  display: inline-flex;
  align-items: center;
  gap: 0.5rem;
...
.color-editor-range-hue { background: transparent; }
